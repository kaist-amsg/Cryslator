...
def get_train_val_test_loader(dataset, collate_fn=default_collate,
                              ...
                              num_workers=0, pin_memory=False, return_test=False):
...
def collate_pool(dataset_list):
		...
		batch_num_nbr = [] ; batch_dij = []
		crystal_atom_idx, batch_target = [], []
		batch_cif_ids = []
		base_idx = 0
		for i, ((atom_fea, nbr_fea, nbr_fea_idx1, nbr_fea_idx2, num_nbr,dij_), target, cif_id)\
            in enumerate(dataset_list):
				...
				batch_cif_ids.append(cif_id) ; batch_dij.append(dij_)
				base_idx += n_i
...
def convert_distance(x):
    ...
    x_max = x[max_index]
    a = 0.2*max_index
    d = torch.sqrt(-1*torch.log(x_max)*(step**2)) + a
    return d
...
class CIFData(Dataset):
		def __init__(self,root_dir,radius=4.0,dmin=0,step=0.2,random_seed=1234):
				self.root_dir = root_dir
				self.radius = radius
				id_prop_file = os.path.join(self.root_dir, 'id_prop_relaxed.csv')
		...
		def __getitem__(self,idx):
				cif_id, target = self.id_prop_data[idx] #; cif_id = cif_id.split('_relaxed')[0]+'_unrelaxed'
				...
				#(p,q) = atom_fea.shape
				#atom_fea += 1.0e-4 * np.random.normal(size=(p,q))
				...
				#(p,) = dij.shape
				#dij += 1.0e-1 * np.random.normal(size=(p,))

   
				nbr_fea = self.gdf.expand(dij)
				#(p,q) = nbr_fea.shape
				#nbr_fea += 1.0e-2 * np.random.normal(size=(p,q))
				...
				return (atom_fea, nbr_fea, nbr_fea_idx1, nbr_fea_idx2, num_nbr,dij_), target, cif_id
            
class CIFData_unrelaxed(Dataset):
		def __init__(self,root_dir,radius=4.0,dmin=0,step=0.2,random_seed=1234):
				self.root_dir = root_dir
				self.radius = radius
